...
class TicketViewSetTest(APITestCase):
    ...
    def test_retrieve_community_ticket_voters(self):
        """
         - retrieve community ticket voters by unauthenticated user
         - retrieve community ticket voters by authenticated user
        """
        # retrieve community ticket voters by unauthenticated user
        response = self.client.get(
            reverse(
                'tickets:ticket-voters',
                kwargs={'pk': self.ticket_by_community_user.id}
            )
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_retrieve_company_ticket_voters(self):
        """
         - retrieve company ticket voters by non permission users
         - retireve company ticket voters by permission users
        """
        # retrieve company ticket voters by non permission users
        non_permission_users = [
            self.community_user,
        ...
        # retrieve company ticket voters by permission users
        permission_users = [
            self.gluu_user, self.gluu_named, self.gluu_admin,
            self.staff, self.manager
        ]
